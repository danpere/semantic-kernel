...
public class CodeBlockTests
{
    ...
    public async Task ItThrowsIfAFunctionCallThrowsAsync()
    {
        ...
        function
            .Setup(x => x.InvokeAsync(It.IsAny<SKContext?>(), It.IsAny<ChatRequestSettings?>(), It.IsAny<ILogger?>(), It.IsAny<CancellationToken?>()))
    ...
    public async Task ItInvokesFunctionCloningAllVariablesAsync()
    {
        ...
        function
            .Setup(x => x.InvokeAsync(It.IsAny<SKContext?>(), It.IsAny<ChatRequestSettings?>(), It.IsAny<ILogger?>(), It.IsAny<CancellationToken?>()))
            .Callback<SKContext?, ChatRequestSettings?, ILogger?, CancellationToken?>((ctx, _, _, _) =>
            {
                canary0 = ctx!["input"];
                canary1 = ctx["var1"];
                canary2 = ctx["var2"];
